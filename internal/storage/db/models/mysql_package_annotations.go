...
func (q packageAnnotationQuery) Upsert(o *PackageAnnotation, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLPackageAnnotationUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`package_annotations`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `package_annotations` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for package_annotations")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == packageAnnotationMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(packageAnnotationType, packageAnnotationMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for package_annotations")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for package_annotations")
	}
...
func FindPackageAnnotation(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*PackageAnnotation, error) {
	...
	query := fmt.Sprintf(
		"select %s from `package_annotations` where `id`=?", sel,
	)
...
func (q packageAnnotationQuery) Insert(o *PackageAnnotation, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `package_annotations` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `package_annotations` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, packageAnnotationPrimaryKeyColumns))
		}
