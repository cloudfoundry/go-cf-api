...
var mySQLRouteBindingOperationUniqueColumns = []string{
	"id",
	"route_binding_id",
}
...
func (q routeBindingOperationQuery) Upsert(o *RouteBindingOperation, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLRouteBindingOperationUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`route_binding_operations`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `route_binding_operations` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for route_binding_operations")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == routeBindingOperationMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(routeBindingOperationType, routeBindingOperationMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for route_binding_operations")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for route_binding_operations")
	}
...
func FindRouteBindingOperation(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RouteBindingOperation, error) {
	...
	query := fmt.Sprintf(
		"select %s from `route_binding_operations` where `id`=?", sel,
	)
...
func (q routeBindingOperationQuery) Insert(o *RouteBindingOperation, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `route_binding_operations` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `route_binding_operations` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, routeBindingOperationPrimaryKeyColumns))
		}
...
func (q routeBindingOperationQuery) UpdateAllSlice(o RouteBindingOperationSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `route_binding_operations` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, routeBindingOperationPrimaryKeyColumns, len(o)))
...
func (q routeBindingOperationQuery) Delete(o *RouteBindingOperation, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `route_binding_operations` WHERE `id`=?"
...
func RouteBindingOperationExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `route_binding_operations` where `id`=? limit 1)"
