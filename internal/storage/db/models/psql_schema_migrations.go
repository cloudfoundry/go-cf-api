...
//go:generate sh -c "echo '\x2bbuild unit' > ../../../../buildtags.txt && mockgen -source=$GOFILE -destination=mocks/schema_migrations.go -copyright_file=../../../../buildtags.txt && rm -f ../../../../buildtags.txt"
...
func FindSchemaMigration(ctx context.Context, exec boil.ContextExecutor, filename string, selectCols ...string) (*SchemaMigration, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"schema_migrations\" where \"filename\"=$1", sel,
	)

	q := queries.Raw(query, filename)
