...
var mySQLServiceBindingOperationUniqueColumns = []string{
	"id",
	"service_binding_id",
}
...
func (q serviceBindingOperationQuery) Upsert(o *ServiceBindingOperation, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLServiceBindingOperationUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`service_binding_operations`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `service_binding_operations` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for service_binding_operations")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == serviceBindingOperationMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(serviceBindingOperationType, serviceBindingOperationMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for service_binding_operations")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for service_binding_operations")
	}
...
func (q serviceBindingOperationQuery) RemoveServiceBinding(o *ServiceBindingOperation, ctx context.Context, exec boil.ContextExecutor, related *ServiceBinding) error {
	...
	if _, err = q.Update(o, ctx, exec, boil.Whitelist("service_binding_id")); err != nil {
		return errors.Wrap(err, "failed to update local table")
	}
...
func FindServiceBindingOperation(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ServiceBindingOperation, error) {
	...
	query := fmt.Sprintf(
		"select %s from `service_binding_operations` where `id`=?", sel,
	)
...
func (q serviceBindingOperationQuery) Insert(o *ServiceBindingOperation, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `service_binding_operations` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `service_binding_operations` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, serviceBindingOperationPrimaryKeyColumns))
		}
...
func (q serviceBindingOperationQuery) UpdateAllSlice(o ServiceBindingOperationSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `service_binding_operations` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, serviceBindingOperationPrimaryKeyColumns, len(o)))
...
func (q serviceBindingOperationQuery) Delete(o *ServiceBindingOperation, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `service_binding_operations` WHERE `id`=?"
...
func ServiceBindingOperationExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `service_binding_operations` where `id`=? limit 1)"
