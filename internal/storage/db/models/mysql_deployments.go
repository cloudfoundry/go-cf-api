...
func (q deploymentQuery) Upsert(o *Deployment, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLDeploymentUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`deployments`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `deployments` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for deployments")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == deploymentMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(deploymentType, deploymentMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for deployments")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for deployments")
	}
...
func FindDeployment(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Deployment, error) {
	...
	query := fmt.Sprintf(
		"select %s from `deployments` where `id`=?", sel,
	)
...
func (q deploymentQuery) Insert(o *Deployment, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `deployments` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `deployments` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, deploymentPrimaryKeyColumns))
		}
