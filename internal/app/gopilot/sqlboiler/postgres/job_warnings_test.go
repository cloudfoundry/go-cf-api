...
var (
	jobWarningDBTypes = map[string]string{`ID`: `integer`, `GUID`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `Detail`: `character varying`, `JobID`: `integer`, `FKJobsID`: `integer`}
	_                 = bytes.MinRead
)
