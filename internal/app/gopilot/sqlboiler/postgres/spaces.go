...
// space replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.TargetSpaceSpaces's ServiceInstances accordingly.
// Replaces o.R.ServiceInstances with related.
// Sets related.R.TargetSpaceSpaces's ServiceInstances accordingly.
func (o *Space) SetServiceInstances(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ServiceInstance) error {
	...
	removeServiceInstancesFromTargetSpaceSpacesSlice(o, related)
	if o.R != nil {
...
// Sets related.R.TargetSpaceSpaces.
func (o *Space) RemoveServiceInstances(ctx context.Context, exec boil.ContextExecutor, related ...*ServiceInstance) error {
	if len(related) == 0 {
		return nil
	}
...
func removeServiceInstancesFromTargetSpaceSpacesSlice(o *Space, related []*ServiceInstance) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
		for i, ri := range rel.R.TargetSpaceSpaces {
			if o.GUID != ri.GUID {
				continue
			}

			ln := len(rel.R.TargetSpaceSpaces)
			if ln > 1 && i < ln-1 {
				rel.R.TargetSpaceSpaces[i] = rel.R.TargetSpaceSpaces[ln-1]
			}
			rel.R.TargetSpaceSpaces = rel.R.TargetSpaceSpaces[:ln-1]
			break
		}
	}
}
