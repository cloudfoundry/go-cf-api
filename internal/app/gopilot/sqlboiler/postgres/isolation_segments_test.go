...
func testIsolationSegmentToManyOrganizationsIsolationSegments(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	var a IsolationSegment
	var b, c OrganizationsIsolationSegment
	...
	if err = randomize.Struct(seed, &b, organizationsIsolationSegmentDBTypes, false, organizationsIsolationSegmentColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, organizationsIsolationSegmentDBTypes, false, organizationsIsolationSegmentColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}

	b.IsolationSegmentGUID = a.GUID
	c.IsolationSegmentGUID = a.GUID
	...
	check, err := a.OrganizationsIsolationSegments().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if v.IsolationSegmentGUID == b.IsolationSegmentGUID {
			bFound = true
		}
		if v.IsolationSegmentGUID == c.IsolationSegmentGUID {
			cFound = true
		}
	}
	...
	slice := IsolationSegmentSlice{&a}
	if err = a.L.LoadOrganizationsIsolationSegments(ctx, tx, false, (*[]*IsolationSegment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OrganizationsIsolationSegments); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.OrganizationsIsolationSegments = nil
	if err = a.L.LoadOrganizationsIsolationSegments(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OrganizationsIsolationSegments); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testIsolationSegmentToManyAddOpOrganizationsIsolationSegments(t *testing.T) {
	var err error
	...
	var a IsolationSegment
	var b, c, d, e OrganizationsIsolationSegment

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, isolationSegmentDBTypes, false, strmangle.SetComplement(isolationSegmentPrimaryKeyColumns, isolationSegmentColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*OrganizationsIsolationSegment{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, organizationsIsolationSegmentDBTypes, false, strmangle.SetComplement(organizationsIsolationSegmentPrimaryKeyColumns, organizationsIsolationSegmentColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*OrganizationsIsolationSegment{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddOrganizationsIsolationSegments(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.GUID != first.IsolationSegmentGUID {
			t.Error("foreign key was wrong value", a.GUID, first.IsolationSegmentGUID)
		}
		if a.GUID != second.IsolationSegmentGUID {
		...
		if a.R.OrganizationsIsolationSegments[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.OrganizationsIsolationSegments[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.OrganizationsIsolationSegments().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	isolationSegmentDBTypes = map[string]string{`ID`: `integer`, `GUID`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `Name`: `USER-DEFINED`}
	_                       = bytes.MinRead
)
