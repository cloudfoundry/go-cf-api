...
var (
	...
	routeColumnsWithoutDefault = []string{"guid", "updated_at", "host", "domain_id", "space_id", "path", "vip_offset"}
	routeColumnsWithDefault    = []string{"id", "created_at", "port"}
...
func (q routeQuery) Insert(o *Route, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `routes` () VALUES ()%s%s"
		}
...
var mySQLRouteUniqueColumns = []string{
	"id",
	"guid",
	"vip_offset",
}
...
func (q routeQuery) Upsert(o *Route, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(routeColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLRouteUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`routes`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `routes` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for routes")
	}
	...
	uniqueMap, err = queries.BindMapping(routeType, routeMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for routes")
	}
