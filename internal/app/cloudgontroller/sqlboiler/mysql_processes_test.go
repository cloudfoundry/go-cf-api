...
var (
	processDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Production`: `tinyint`, `Memory`: `int`, `Instances`: `int`, `FileDescriptors`: `int`, `DiskQuota`: `int`, `State`: `varchar`, `Version`: `varchar`, `Metadata`: `varchar`, `DetectedBuildpack`: `varchar`, `NotDeleted`: `tinyint`, `HealthCheckTimeout`: `int`, `Diego`: `tinyint`, `PackageUpdatedAt`: `datetime`, `AppGUID`: `varchar`, `Type`: `varchar`, `HealthCheckType`: `varchar`, `Command`: `varchar`, `EnableSSH`: `tinyint`, `EncryptedDockerCredentialsJSON`: `varchar`, `DockerSalt`: `varchar`, `Ports`: `text`, `HealthCheckHTTPEndpoint`: `text`, `HealthCheckInvocationTimeout`: `int`, `RevisionGUID`: `varchar`}
	_              = bytes.MinRead
)
...
func testProcessesUpsert(t *testing.T) {
	...
	if len(mySQLProcessUniqueColumns) == 0 {
