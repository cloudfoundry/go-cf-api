...
var (
	domainLabelDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `ResourceGUID`: `varchar`, `KeyPrefix`: `varchar`, `KeyName`: `varchar`, `Value`: `varchar`}
	_                  = bytes.MinRead
)
...
func testDomainLabelsUpsert(t *testing.T) {
	...
	if len(mySQLDomainLabelUniqueColumns) == 0 {
