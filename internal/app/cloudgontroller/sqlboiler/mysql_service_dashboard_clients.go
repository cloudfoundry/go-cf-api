...
func FindServiceDashboardClient(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ServiceDashboardClient, error) {
	...
	query := fmt.Sprintf(
		"select %s from `service_dashboard_clients` where `id`=?", sel,
	)
...
func (q serviceDashboardClientQuery) Insert(o *ServiceDashboardClient, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `service_dashboard_clients` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `service_dashboard_clients` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, serviceDashboardClientPrimaryKeyColumns))
		}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == serviceDashboardClientMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for service_dashboard_clients")
	}
...
func (q serviceDashboardClientQuery) Update(o *ServiceDashboardClient, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE `service_dashboard_clients` SET %s WHERE %s",
			...
			strmangle.WhereClause("`", "`", 0, serviceDashboardClientPrimaryKeyColumns),
		)
...
func (q serviceDashboardClientQuery) Upsert(o *ServiceDashboardClient, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(serviceDashboardClientColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLServiceDashboardClientUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`service_dashboard_clients`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `service_dashboard_clients` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for service_dashboard_clients")
	}
	...
	uniqueMap, err = queries.BindMapping(serviceDashboardClientType, serviceDashboardClientMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for service_dashboard_clients")
	}
