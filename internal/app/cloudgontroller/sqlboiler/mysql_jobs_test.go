...
var (
	jobDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `State`: `varchar`, `Operation`: `varchar`, `ResourceGUID`: `varchar`, `ResourceType`: `varchar`, `DelayedJobGUID`: `varchar`, `CFAPIError`: `varchar`}
	_          = bytes.MinRead
)
...
func testJobsUpsert(t *testing.T) {
	...
	if len(mySQLJobUniqueColumns) == 0 {
