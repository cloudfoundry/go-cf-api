...
var (
	organizationAnnotationDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `ResourceGUID`: `varchar`, `KeyPrefix`: `varchar`, `Key`: `varchar`, `Value`: `varchar`}
	_                             = bytes.MinRead
)
...
func testOrganizationAnnotationsUpsert(t *testing.T) {
	...
	if len(mySQLOrganizationAnnotationUniqueColumns) == 0 {
