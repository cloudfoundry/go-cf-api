...
func (q revisionProcessCommandQuery) Upsert(o *RevisionProcessCommand, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLRevisionProcessCommandUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`revision_process_commands`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `revision_process_commands` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for revision_process_commands")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == revisionProcessCommandMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(revisionProcessCommandType, revisionProcessCommandMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for revision_process_commands")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for revision_process_commands")
	}
...
func FindRevisionProcessCommand(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RevisionProcessCommand, error) {
	...
	query := fmt.Sprintf(
		"select %s from `revision_process_commands` where `id`=?", sel,
	)
...
func (q revisionProcessCommandQuery) Insert(o *RevisionProcessCommand, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `revision_process_commands` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `revision_process_commands` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, revisionProcessCommandPrimaryKeyColumns))
		}
