...
var (
	quotaDefinitionDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Name`: `varchar`, `NonBasicServicesAllowed`: `tinyint`, `TotalServices`: `int`, `MemoryLimit`: `int`, `TotalRoutes`: `int`, `InstanceMemoryLimit`: `int`, `TotalPrivateDomains`: `int`, `AppInstanceLimit`: `int`, `AppTaskLimit`: `int`, `TotalServiceKeys`: `int`, `TotalReservedRoutePorts`: `int`}
	_                      = bytes.MinRead
)
...
func testQuotaDefinitionsUpsert(t *testing.T) {
	...
	if len(mySQLQuotaDefinitionUniqueColumns) == 0 {
