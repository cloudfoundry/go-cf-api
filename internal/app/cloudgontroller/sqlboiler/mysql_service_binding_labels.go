...
func FindServiceBindingLabel(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ServiceBindingLabel, error) {
	...
	query := fmt.Sprintf(
		"select %s from `service_binding_labels` where `id`=?", sel,
	)
...
func (o *ServiceBindingLabel) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `service_binding_labels` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `service_binding_labels` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, serviceBindingLabelPrimaryKeyColumns))
		}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == serviceBindingLabelMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for service_binding_labels")
	}
...
func (o ServiceBindingLabelSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `service_binding_labels` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, serviceBindingLabelPrimaryKeyColumns, len(o)))
...
var mySQLServiceBindingLabelUniqueColumns = []string{
	"id",
	"guid",
}
...
func (o *ServiceBindingLabel) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(serviceBindingLabelColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLServiceBindingLabelUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`service_binding_labels`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `service_binding_labels` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for service_binding_labels")
	}
	...
	uniqueMap, err = queries.BindMapping(serviceBindingLabelType, serviceBindingLabelMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for service_binding_labels")
	}
...
func (o *ServiceBindingLabel) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `service_binding_labels` WHERE `id`=?"
...
func ServiceBindingLabelExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `service_binding_labels` where `id`=? limit 1)"
