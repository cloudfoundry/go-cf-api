...
func testSpaceToManySharedServiceInstances(t *testing.T) {
	...
	_, err = tx.Exec("insert into `service_instance_shares` (`target_space_guid`, `service_instance_guid`) values (?, ?)", a.GUID, b.GUID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into `service_instance_shares` (`target_space_guid`, `service_instance_guid`) values (?, ?)", a.GUID, c.GUID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	spaceDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Name`: `varchar`, `OrganizationID`: `int`, `SpaceQuotaDefinitionID`: `int`, `AllowSSH`: `tinyint`, `IsolationSegmentGUID`: `varchar`}
	_            = bytes.MinRead
)
