...
func testOrganizationsManagerToOneOrganizationUsingOrganization(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOrganization(ctx, tx, false, (*[]*OrganizationsManager)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOrganizationsManagerToOneSetOpOrganizationUsingOrganization(t *testing.T) {
	var err error
...
var (
	organizationsManagerDBTypes = map[string]string{`OrganizationID`: `int`, `UserID`: `int`, `OrganizationsManagersPK`: `int`, `RoleGUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_                           = bytes.MinRead
)
...
func testOrganizationsManagersUpsert(t *testing.T) {
	...
	if len(mySQLOrganizationsManagerUniqueColumns) == 0 {
