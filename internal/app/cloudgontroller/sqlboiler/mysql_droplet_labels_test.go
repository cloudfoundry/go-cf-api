...
var (
	dropletLabelDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `ResourceGUID`: `varchar`, `KeyPrefix`: `varchar`, `KeyName`: `varchar`, `Value`: `varchar`}
	_                   = bytes.MinRead
)
...
func testDropletLabelsUpsert(t *testing.T) {
	...
	if len(mySQLDropletLabelUniqueColumns) == 0 {
