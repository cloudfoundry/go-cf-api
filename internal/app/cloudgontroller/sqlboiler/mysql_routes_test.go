...
var (
	routeDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Host`: `varchar`, `DomainID`: `int`, `SpaceID`: `int`, `Path`: `varchar`, `Port`: `int`, `VipOffset`: `int`}
	_            = bytes.MinRead
)
...
func testRoutesUpsert(t *testing.T) {
	...
	if len(mySQLRouteUniqueColumns) == 0 {
