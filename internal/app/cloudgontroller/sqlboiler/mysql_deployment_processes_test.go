...
var (
	deploymentProcessDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `ProcessGUID`: `varchar`, `ProcessType`: `varchar`, `DeploymentGUID`: `varchar`}
	_                        = bytes.MinRead
)
...
func testDeploymentProcessesUpsert(t *testing.T) {
	...
	if len(mySQLDeploymentProcessUniqueColumns) == 0 {
