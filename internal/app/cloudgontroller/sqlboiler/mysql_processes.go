...
var mySQLProcessUniqueColumns = []string{
	"id",
	"guid",
}
...
func (q processQuery) Upsert(o *Process, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLProcessUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`processes`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `processes` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for processes")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == processMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(processType, processMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for processes")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for processes")
	}
...
func (q processQuery) SetResourceProcessAnnotations(o *Process, ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ProcessAnnotation) error {
	query := "update `process_annotations` set `resource_guid` = null where `resource_guid` = ?"
...
func (q processQuery) SetResourceProcessLabels(o *Process, ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ProcessLabel) error {
	query := "update `process_labels` set `resource_guid` = null where `resource_guid` = ?"
	values := []interface{}{o.GUID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
...
func FindProcess(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Process, error) {
	...
	query := fmt.Sprintf(
		"select %s from `processes` where `id`=?", sel,
	)
...
func (q processQuery) Insert(o *Process, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `processes` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `processes` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, processPrimaryKeyColumns))
		}
...
func (q processQuery) UpdateAllSlice(o ProcessSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `processes` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, processPrimaryKeyColumns, len(o)))
...
func ProcessExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `processes` where `id`=? limit 1)"
