...
var mySQLEncryptionKeySentinelUniqueColumns = []string{
	"id",
	"guid",
...
func (q encryptionKeySentinelQuery) Upsert(o *EncryptionKeySentinel, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLEncryptionKeySentinelUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`encryption_key_sentinels`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `encryption_key_sentinels` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for encryption_key_sentinels")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == encryptionKeySentinelMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(encryptionKeySentinelType, encryptionKeySentinelMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for encryption_key_sentinels")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for encryption_key_sentinels")
	}
...
func FindEncryptionKeySentinel(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*EncryptionKeySentinel, error) {
	...
	query := fmt.Sprintf(
		"select %s from `encryption_key_sentinels` where `id`=?", sel,
	)
...
func (q encryptionKeySentinelQuery) Insert(o *EncryptionKeySentinel, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `encryption_key_sentinels` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `encryption_key_sentinels` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, encryptionKeySentinelPrimaryKeyColumns))
		}
...
func (q encryptionKeySentinelQuery) Update(o *EncryptionKeySentinel, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE `encryption_key_sentinels` SET %s WHERE %s",
			...
			strmangle.WhereClause("`", "`", 0, encryptionKeySentinelPrimaryKeyColumns),
		)
...
func (q encryptionKeySentinelQuery) UpdateAllSlice(o EncryptionKeySentinelSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `encryption_key_sentinels` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, encryptionKeySentinelPrimaryKeyColumns, len(o)))
...
func (q encryptionKeySentinelQuery) Delete(o *EncryptionKeySentinel, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `encryption_key_sentinels` WHERE `id`=?"
...
func EncryptionKeySentinelExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `encryption_key_sentinels` where `id`=? limit 1)"
