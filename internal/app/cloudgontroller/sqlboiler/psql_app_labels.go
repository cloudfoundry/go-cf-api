//go:generate mockgen -source=$GOFILE -destination=mocks/app_labels.go
// +build psql
...
func FindAppLabel(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*AppLabel, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"app_labels\" where \"id\"=$1", sel,
	)
...
func (q AppLabelQuery) Insert(o *AppLabel, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"app_labels\" %sDEFAULT VALUES%s"
		}
...
func (q AppLabelQuery) Update(o *AppLabel, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"app_labels\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, appLabelPrimaryKeyColumns),
		)
...
func (q AppLabelQuery) UpdateAllSlice(o AppLabelSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"app_labels\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, appLabelPrimaryKeyColumns, len(o)))
...
func (q AppLabelQuery) Delete(o *AppLabel, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"app_labels\" WHERE \"id\"=$1"
...
func (q AppLabelQuery) DeleteAllSlice(o AppLabelSlice, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"app_labels\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, appLabelPrimaryKeyColumns, len(o))
...
func AppLabelExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"app_labels\" where \"id\"=$1 limit 1)"
...
func (o *AppLabel) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(appLabelPrimaryKeyColumns))
			copy(conflict, appLabelPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"app_labels\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert app_labels")
	}
