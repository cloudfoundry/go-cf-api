...
func (q isolationSegmentQuery) AddOrganizations(o *IsolationSegment, ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Organization) error {
	...
	for _, rel := range related {
		query := "insert into `organizations_isolation_segments` (`isolation_segment_guid`, `organization_guid`) values (?, ?)"
...
func (q isolationSegmentQuery) RemoveOrganizations(o *IsolationSegment, ctx context.Context, exec boil.ContextExecutor, related ...*Organization) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from `organizations_isolation_segments` where `isolation_segment_guid` = ? and `organization_guid` in (%s)",
...
func FindIsolationSegment(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*IsolationSegment, error) {
	...
	query := fmt.Sprintf(
		"select %s from `isolation_segments` where `id`=?", sel,
	)
...
func (q isolationSegmentQuery) Insert(o *IsolationSegment, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `isolation_segments` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `isolation_segments` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, isolationSegmentPrimaryKeyColumns))
		}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == isolationSegmentMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for isolation_segments")
	}
...
func (q isolationSegmentQuery) Update(o *IsolationSegment, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE `isolation_segments` SET %s WHERE %s",
			...
			strmangle.WhereClause("`", "`", 0, isolationSegmentPrimaryKeyColumns),
		)
...
func (q isolationSegmentQuery) Upsert(o *IsolationSegment, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(isolationSegmentColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLIsolationSegmentUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`isolation_segments`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `isolation_segments` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for isolation_segments")
	}
	...
	uniqueMap, err = queries.BindMapping(isolationSegmentType, isolationSegmentMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for isolation_segments")
	}
