...
var (
	spacesManagerDBTypes = map[string]string{`SpaceID`: `int`, `UserID`: `int`, `SpacesManagersPK`: `int`, `RoleGUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_                    = bytes.MinRead
)
...
func testSpacesManagersUpsert(t *testing.T) {
	...
	if len(mySQLSpacesManagerUniqueColumns) == 0 {
