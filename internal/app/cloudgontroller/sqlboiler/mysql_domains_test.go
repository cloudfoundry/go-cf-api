...
func testDomainToOneOrganizationUsingOwningOrganization(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.OwningOrganizationID, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.OwningOrganization().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadOwningOrganization(ctx, tx, false, (*[]*Domain)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.OwningOrganization == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.OwningOrganization = nil
	if err = local.L.LoadOwningOrganization(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.OwningOrganization == nil {
		t.Error("struct should have been eager loaded")
...
func testDomainToOneSetOpOrganizationUsingOwningOrganization(t *testing.T) {
	var err error
	...
	for i, x := range []*Organization{&b, &c} {
		err = a.SetOwningOrganization(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.OwningOrganization != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.OwningOrganizationDomains[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.OwningOrganizationID, x.ID) {
			t.Error("foreign key was wrong value", a.OwningOrganizationID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.OwningOrganizationID))
		reflect.Indirect(reflect.ValueOf(&a.OwningOrganizationID)).Set(zero)
		...
		if !queries.Equal(a.OwningOrganizationID, x.ID) {
			t.Error("foreign key was wrong value", a.OwningOrganizationID, x.ID)
		}
	}
}

func testDomainToOneRemoveOpOrganizationUsingOwningOrganization(t *testing.T) {
	var err error
	...
	if err = a.SetOwningOrganization(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveOwningOrganization(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.OwningOrganization().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if a.R.OwningOrganization != nil {
		t.Error("R struct entry should be nil")
	}

	if !queries.IsValuerNil(a.OwningOrganizationID) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.OwningOrganizationDomains) != 0 {
...
var (
	domainDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Name`: `varchar`, `Wildcard`: `tinyint`, `OwningOrganizationID`: `int`, `RouterGroupGUID`: `varchar`, `Internal`: `tinyint`}
	_             = bytes.MinRead
)
