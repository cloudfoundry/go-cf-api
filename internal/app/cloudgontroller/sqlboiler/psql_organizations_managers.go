//go:generate mockgen -source=$GOFILE -destination=mocks/organizations_managers.go
// +build psql
...
func FindOrganizationsManager(ctx context.Context, exec boil.ContextExecutor, organizationsManagersPK int, selectCols ...string) (*OrganizationsManager, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"organizations_managers\" where \"organizations_managers_pk\"=$1", sel,
	)
...
func (q organizationsManagerQuery) Insert(o *OrganizationsManager, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"organizations_managers\" %sDEFAULT VALUES%s"
		}
...
func (q organizationsManagerQuery) Update(o *OrganizationsManager, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"organizations_managers\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, organizationsManagerPrimaryKeyColumns),
		)
...
func (q organizationsManagerQuery) UpdateAllSlice(o OrganizationsManagerSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"organizations_managers\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, organizationsManagerPrimaryKeyColumns, len(o)))
...
func (q organizationsManagerQuery) Delete(o *OrganizationsManager, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"organizations_managers\" WHERE \"organizations_managers_pk\"=$1"
...
func (q organizationsManagerQuery) DeleteAllSlice(o OrganizationsManagerSlice, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"organizations_managers\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, organizationsManagerPrimaryKeyColumns, len(o))
...
func OrganizationsManagerExists(ctx context.Context, exec boil.ContextExecutor, organizationsManagersPK int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"organizations_managers\" where \"organizations_managers_pk\"=$1 limit 1)"
...
func (o *OrganizationsManager) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(organizationsManagerPrimaryKeyColumns))
			copy(conflict, organizationsManagerPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"organizations_managers\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert organizations_managers")
	}
