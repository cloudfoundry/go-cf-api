...
var (
	delayedJobDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Priority`: `int`, `Attempts`: `int`, `Handler`: `longtext`, `LastError`: `text`, `RunAt`: `datetime`, `LockedAt`: `datetime`, `FailedAt`: `datetime`, `LockedBy`: `varchar`, `Queue`: `varchar`, `CFAPIError`: `text`}
	_                 = bytes.MinRead
)
...
func testDelayedJobsUpsert(t *testing.T) {
	...
	if len(mySQLDelayedJobUniqueColumns) == 0 {
