...
var (
	spacesAuditorDBTypes = map[string]string{`SpaceID`: `int`, `UserID`: `int`, `SpacesAuditorsPK`: `int`, `RoleGUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_                    = bytes.MinRead
)
...
func testSpacesAuditorsUpsert(t *testing.T) {
	...
	if len(mySQLSpacesAuditorUniqueColumns) == 0 {
