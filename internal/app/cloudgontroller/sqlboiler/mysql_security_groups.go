...
func (o *SecurityGroup) SecurityGroupsSpaces(mods ...qm.QueryMod) securityGroupsSpaceQuery {
...
func (o *SecurityGroup) StagingSecurityGroupStagingSecurityGroupsSpaces(mods ...qm.QueryMod) stagingSecurityGroupsSpaceQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
func (o *SecurityGroup) AddSecurityGroupsSpaces(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*SecurityGroupsSpace) error {
	var err error
...
func (o *SecurityGroup) AddStagingSecurityGroupStagingSecurityGroupsSpaces(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*StagingSecurityGroupsSpace) error {
	var err error
	for _, rel := range related {
		if insert {
...
func FindSecurityGroup(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*SecurityGroup, error) {
	...
	query := fmt.Sprintf(
		"select %s from `security_groups` where `id`=?", sel,
	)
...
func (o *SecurityGroup) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `security_groups` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `security_groups` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, securityGroupPrimaryKeyColumns))
		}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == securityGroupMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for security_groups")
	}
...
func (o *SecurityGroup) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE `security_groups` SET %s WHERE %s",
			...
			strmangle.WhereClause("`", "`", 0, securityGroupPrimaryKeyColumns),
		)
...
func (o SecurityGroupSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `security_groups` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, securityGroupPrimaryKeyColumns, len(o)))
...
var mySQLSecurityGroupUniqueColumns = []string{
	"id",
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *SecurityGroup) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(securityGroupColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLSecurityGroupUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`security_groups`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `security_groups` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for security_groups")
	}
	...
	uniqueMap, err = queries.BindMapping(securityGroupType, securityGroupMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for security_groups")
	}
...
func (o *SecurityGroup) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `security_groups` WHERE `id`=?"
...
func SecurityGroupExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `security_groups` where `id`=? limit 1)"
