...
var (
	encryptionKeySentinelDBTypes = map[string]string{`ID`: `integer`, `GUID`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `ExpectedValue`: `character varying`, `EncryptedValue`: `character varying`, `EncryptionKeyLabel`: `character varying`, `Salt`: `character varying`, `EncryptionIterations`: `integer`}
	_                            = bytes.MinRead
)
