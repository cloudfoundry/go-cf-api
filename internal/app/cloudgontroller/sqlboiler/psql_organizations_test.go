...
func testOrganizationToManyIsolationSegments(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"organizations_isolation_segments\" (\"organization_guid\", \"isolation_segment_guid\") values ($1, $2)", a.GUID, b.GUID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"organizations_isolation_segments\" (\"organization_guid\", \"isolation_segment_guid\") values ($1, $2)", a.GUID, c.GUID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	organizationDBTypes = map[string]string{`ID`: `integer`, `GUID`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `Name`: `USER-DEFINED`, `BillingEnabled`: `boolean`, `QuotaDefinitionID`: `integer`, `Status`: `text`, `DefaultIsolationSegmentGUID`: `text`}
	_                   = bytes.MinRead
)
