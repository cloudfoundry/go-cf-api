...
var (
	organizationDBTypes = map[string]string{`ID`: `integer`, `GUID`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `Name`: `USER-DEFINED`, `BillingEnabled`: `boolean`, `QuotaDefinitionID`: `integer`, `Status`: `text`}
	_                   = bytes.MinRead
)
