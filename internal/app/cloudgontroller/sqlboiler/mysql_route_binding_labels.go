...
var mySQLRouteBindingLabelUniqueColumns = []string{
	"id",
	"guid",
}
...
func (q routeBindingLabelQuery) Upsert(o *RouteBindingLabel, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLRouteBindingLabelUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`route_binding_labels`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `route_binding_labels` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for route_binding_labels")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == routeBindingLabelMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(routeBindingLabelType, routeBindingLabelMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for route_binding_labels")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for route_binding_labels")
	}
...
func FindRouteBindingLabel(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RouteBindingLabel, error) {
	...
	query := fmt.Sprintf(
		"select %s from `route_binding_labels` where `id`=?", sel,
	)
...
func (q routeBindingLabelQuery) Insert(o *RouteBindingLabel, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `route_binding_labels` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `route_binding_labels` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, routeBindingLabelPrimaryKeyColumns))
		}
...
func (q routeBindingLabelQuery) UpdateAllSlice(o RouteBindingLabelSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `route_binding_labels` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, routeBindingLabelPrimaryKeyColumns, len(o)))
...
func (q routeBindingLabelQuery) Delete(o *RouteBindingLabel, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `route_binding_labels` WHERE `id`=?"
...
func RouteBindingLabelExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `route_binding_labels` where `id`=? limit 1)"
