...
func (q serviceBindingQuery) Upsert(o *ServiceBinding, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLServiceBindingUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`service_bindings`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `service_bindings` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for service_bindings")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == serviceBindingMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(serviceBindingType, serviceBindingMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for service_bindings")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for service_bindings")
	}
...
func (q serviceBindingQuery) SetServiceBindingOperation(o *ServiceBinding, ctx context.Context, exec boil.ContextExecutor, insert bool, related *ServiceBindingOperation) error {
	...
	if insert {
		...
		updateQuery := fmt.Sprintf(
			...
			strmangle.SetParamNames("`", "`", 0, []string{"service_binding_id"}),
			strmangle.WhereClause("`", "`", 0, serviceBindingOperationPrimaryKeyColumns),
		)
	...
	for _, rel := range related {
		if insert {
			...
			updateQuery := fmt.Sprintf(
				...
				strmangle.SetParamNames("`", "`", 0, []string{"resource_guid"}),
				strmangle.WhereClause("`", "`", 0, serviceBindingLabelPrimaryKeyColumns),
			)
			values := []interface{}{o.GUID, rel.ID}
...
func FindServiceBinding(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ServiceBinding, error) {
	...
	query := fmt.Sprintf(
		"select %s from `service_bindings` where `id`=?", sel,
	)
...
func (q serviceBindingQuery) Insert(o *ServiceBinding, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `service_bindings` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `service_bindings` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, serviceBindingPrimaryKeyColumns))
		}
