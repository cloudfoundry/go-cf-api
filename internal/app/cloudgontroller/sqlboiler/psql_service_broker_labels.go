// +build psql
//go:generate mockgen -source=$GOFILE -destination=mocks/service_broker_labels.go
...
func FindServiceBrokerLabel(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ServiceBrokerLabel, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"service_broker_labels\" where \"id\"=$1", sel,
	)
...
func (q ServiceBrokerLabelQuery) Insert(o *ServiceBrokerLabel, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"service_broker_labels\" %sDEFAULT VALUES%s"
		}
...
func (q ServiceBrokerLabelQuery) Update(o *ServiceBrokerLabel, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"service_broker_labels\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, serviceBrokerLabelPrimaryKeyColumns),
		)
...
func (q ServiceBrokerLabelQuery) UpdateAllSlice(o ServiceBrokerLabelSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"service_broker_labels\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, serviceBrokerLabelPrimaryKeyColumns, len(o)))
...
func (q ServiceBrokerLabelQuery) Delete(o *ServiceBrokerLabel, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"service_broker_labels\" WHERE \"id\"=$1"
...
func (q ServiceBrokerLabelQuery) DeleteAllSlice(o ServiceBrokerLabelSlice, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"service_broker_labels\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, serviceBrokerLabelPrimaryKeyColumns, len(o))
...
func ServiceBrokerLabelExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"service_broker_labels\" where \"id\"=$1 limit 1)"
...
func (o *ServiceBrokerLabel) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(serviceBrokerLabelPrimaryKeyColumns))
			copy(conflict, serviceBrokerLabelPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"service_broker_labels\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert service_broker_labels")
	}
