...
func testServiceInstanceToManyTargetSpaceSpaces(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.TargetSpaceSpaces().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ServiceInstanceSlice{&a}
	if err = a.L.LoadTargetSpaceSpaces(ctx, tx, false, (*[]*ServiceInstance)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TargetSpaceSpaces); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TargetSpaceSpaces = nil
	if err = a.L.LoadTargetSpaceSpaces(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TargetSpaceSpaces); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testServiceInstanceToManyAddOpTargetSpaceSpaces(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddTargetSpaceSpaces(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if first.R.ServiceInstances[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}
		if second.R.ServiceInstances[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}

		if a.R.TargetSpaceSpaces[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TargetSpaceSpaces[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TargetSpaceSpaces().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testServiceInstanceToManySetOpTargetSpaceSpaces(t *testing.T) {
	var err error
	...
	err = a.SetTargetSpaceSpaces(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetTargetSpaceSpaces(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	// }
	// if len(c.R.ServiceInstances) != 0 {
	// 	t.Error("relationship was not removed properly from the slice")
	// }
	if d.R.ServiceInstances[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}
	if e.R.ServiceInstances[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}

	if a.R.TargetSpaceSpaces[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.TargetSpaceSpaces[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testServiceInstanceToManyRemoveOpTargetSpaceSpaces(t *testing.T) {
	var err error
	...
	err = a.AddTargetSpaceSpaces(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveTargetSpaceSpaces(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.TargetSpaceSpaces) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.TargetSpaceSpaces[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.TargetSpaceSpaces[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}
