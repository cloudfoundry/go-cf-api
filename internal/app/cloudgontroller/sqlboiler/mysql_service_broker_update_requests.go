...
func (q serviceBrokerUpdateRequestQuery) Upsert(o *ServiceBrokerUpdateRequest, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLServiceBrokerUpdateRequestUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`service_broker_update_requests`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `service_broker_update_requests` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for service_broker_update_requests")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == serviceBrokerUpdateRequestMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(serviceBrokerUpdateRequestType, serviceBrokerUpdateRequestMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for service_broker_update_requests")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for service_broker_update_requests")
	}
...
func FindServiceBrokerUpdateRequest(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ServiceBrokerUpdateRequest, error) {
	...
	query := fmt.Sprintf(
		"select %s from `service_broker_update_requests` where `id`=?", sel,
	)
...
func (q serviceBrokerUpdateRequestQuery) Insert(o *ServiceBrokerUpdateRequest, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `service_broker_update_requests` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `service_broker_update_requests` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, serviceBrokerUpdateRequestPrimaryKeyColumns))
		}
