...
var (
	securityGroupDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Name`: `varchar`, `Rules`: `mediumtext`, `StagingDefault`: `tinyint`, `RunningDefault`: `tinyint`}
	_                    = bytes.MinRead
)
...
func testSecurityGroupsUpsert(t *testing.T) {
	...
	if len(mySQLSecurityGroupUniqueColumns) == 0 {
