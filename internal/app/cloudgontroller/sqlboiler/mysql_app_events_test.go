...
var (
	appEventDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `AppID`: `int`, `InstanceGUID`: `varchar`, `InstanceIndex`: `int`, `ExitStatus`: `int`, `Timestamp`: `datetime`, `ExitDescription`: `text`}
	_               = bytes.MinRead
)
...
func testAppEventsUpsert(t *testing.T) {
	...
	if len(mySQLAppEventUniqueColumns) == 0 {
