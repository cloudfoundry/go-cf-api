...
func (o *Job) FKJobJobWarnings(mods ...qm.QueryMod) jobWarningQuery {
...
func (o *Job) AddFKJobJobWarnings(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*JobWarning) error {
	var err error
...
func (o *Job) SetFKJobJobWarnings(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*JobWarning) error {
	...
	return o.AddFKJobJobWarnings(ctx, exec, insert, related...)
}
...
func (o *Job) RemoveFKJobJobWarnings(ctx context.Context, exec boil.ContextExecutor, related ...*JobWarning) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("fk_jobs_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
...
func FindJob(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Job, error) {
	...
	query := fmt.Sprintf(
		"select %s from `jobs` where `id`=?", sel,
	)
...
func (o *Job) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `jobs` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `jobs` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, jobPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for jobs")
	}
...
var mySQLJobUniqueColumns = []string{
	"id",
	"guid",
}
...
func (o *Job) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(jobColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLJobUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`jobs`", update, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for jobs")
	}
	...
	uniqueMap, err = queries.BindMapping(jobType, jobMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for jobs")
	}
...
func JobExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `jobs` where `id`=? limit 1)"
