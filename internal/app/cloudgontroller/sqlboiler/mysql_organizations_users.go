...
func (o *OrganizationsUser) Organization(mods ...qm.QueryMod) organizationQuery {
...
func (o *OrganizationsUser) SetOrganization(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Organization) error {
	var err error
	if insert {
...
func FindOrganizationsUser(ctx context.Context, exec boil.ContextExecutor, organizationsUsersPK int, selectCols ...string) (*OrganizationsUser, error) {
	...
	query := fmt.Sprintf(
		"select %s from `organizations_users` where `organizations_users_pk`=?", sel,
	)
...
func (o *OrganizationsUser) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `organizations_users` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `organizations_users` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, organizationsUserPrimaryKeyColumns))
		}
	...
	o.OrganizationsUsersPK = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == organizationsUserMapping["organizations_users_pk"] {
		goto CacheNoHooks
	}

	identifierCols = []interface{}{
		o.OrganizationsUsersPK,
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for organizations_users")
	}
...
func (o OrganizationsUserSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `organizations_users` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, organizationsUserPrimaryKeyColumns, len(o)))
...
var mySQLOrganizationsUserUniqueColumns = []string{
	"organizations_users_pk",
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *OrganizationsUser) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(organizationsUserColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLOrganizationsUserUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`organizations_users`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `organizations_users` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for organizations_users")
	}
	...
	uniqueMap, err = queries.BindMapping(organizationsUserType, organizationsUserMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for organizations_users")
	}
...
func (o *OrganizationsUser) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `organizations_users` WHERE `organizations_users_pk`=?"
...
func OrganizationsUserExists(ctx context.Context, exec boil.ContextExecutor, organizationsUsersPK int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `organizations_users` where `organizations_users_pk`=? limit 1)"
