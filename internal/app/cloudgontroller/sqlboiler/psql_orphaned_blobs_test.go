...
var (
	orphanedBlobDBTypes = map[string]string{`ID`: `integer`, `GUID`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `BlobKey`: `text`, `DirtyCount`: `integer`, `BlobstoreType`: `text`}
	_                   = bytes.MinRead
)
