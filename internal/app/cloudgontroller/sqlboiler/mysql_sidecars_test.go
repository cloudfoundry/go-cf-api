...
var (
	sidecarDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Name`: `varchar`, `Command`: `varchar`, `AppGUID`: `varchar`, `Memory`: `int`, `Origin`: `varchar`}
	_              = bytes.MinRead
)
...
func testSidecarsUpsert(t *testing.T) {
	...
	if len(mySQLSidecarUniqueColumns) == 0 {
