...
func testSchemaMigrationsExists(t *testing.T) {
	...
	e, err := SchemaMigrationExists(ctx, tx, o.Filename)
	if err != nil {
...
func testSchemaMigrationsFind(t *testing.T) {
	...
	schemaMigrationFound, err := FindSchemaMigration(ctx, tx, o.Filename)
	if err != nil {
		t.Error(err)
	}
...
var (
	schemaMigrationDBTypes = map[string]string{`Filename`: `varchar`}
	_                      = bytes.MinRead
)
