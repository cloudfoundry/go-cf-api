...
var mySQLServiceKeyLabelUniqueColumns = []string{
	"id",
	"guid",
}
...
func (q serviceKeyLabelQuery) Upsert(o *ServiceKeyLabel, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzUniques := queries.NonZeroDefaultSet(mySQLServiceKeyLabelUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`service_key_labels`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `service_key_labels` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for service_key_labels")
	}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == serviceKeyLabelMapping["id"] {
		goto CacheNoHooks
	}

	uniqueMap, err = queries.BindMapping(serviceKeyLabelType, serviceKeyLabelMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for service_key_labels")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for service_key_labels")
	}
...
func FindServiceKeyLabel(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ServiceKeyLabel, error) {
	...
	query := fmt.Sprintf(
		"select %s from `service_key_labels` where `id`=?", sel,
	)
...
func (q serviceKeyLabelQuery) Insert(o *ServiceKeyLabel, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `service_key_labels` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `service_key_labels` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, serviceKeyLabelPrimaryKeyColumns))
		}
...
func (q serviceKeyLabelQuery) UpdateAllSlice(o ServiceKeyLabelSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `service_key_labels` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, serviceKeyLabelPrimaryKeyColumns, len(o)))
...
func (q serviceKeyLabelQuery) Delete(o *ServiceKeyLabel, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `service_key_labels` WHERE `id`=?"
...
func ServiceKeyLabelExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `service_key_labels` where `id`=? limit 1)"
