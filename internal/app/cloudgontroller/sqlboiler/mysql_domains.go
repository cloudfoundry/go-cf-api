...
func (q domainQuery) SetOwningOrganization(o *Domain, ctx context.Context, exec boil.ContextExecutor, insert bool, related *Organization) error {
	...
	updateQuery := fmt.Sprintf(
		...
		strmangle.SetParamNames("`", "`", 0, []string{"owning_organization_id"}),
...
func FindDomain(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Domain, error) {
	...
	query := fmt.Sprintf(
		"select %s from `domains` where `id`=?", sel,
	)
...
func (q domainQuery) Insert(o *Domain, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `domains` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `domains` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, domainPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for domains")
	}
...
func (q domainQuery) Upsert(o *Domain, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`domains`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `domains` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for domains")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for domains")
	}
...
func DomainExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `domains` where `id`=? limit 1)"
