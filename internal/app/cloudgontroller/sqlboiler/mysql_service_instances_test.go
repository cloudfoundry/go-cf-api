...
func testServiceInstanceToManySpaces(t *testing.T) {
	...
	_, err = tx.Exec("insert into `service_instance_shares` (`service_instance_guid`, `target_space_guid`) values (?, ?)", a.GUID, b.GUID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into `service_instance_shares` (`service_instance_guid`, `target_space_guid`) values (?, ?)", a.GUID, c.GUID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	serviceInstanceDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Name`: `varchar`, `Credentials`: `text`, `GatewayName`: `varchar`, `GatewayData`: `varchar`, `SpaceID`: `int`, `ServicePlanID`: `int`, `Salt`: `varchar`, `DashboardURL`: `varchar`, `IsGatewayService`: `tinyint`, `SyslogDrainURL`: `text`, `Tags`: `text`, `RouteServiceURL`: `varchar`, `EncryptionKeyLabel`: `varchar`, `EncryptionIterations`: `int`, `MaintenanceInfo`: `text`}
	_                      = bytes.MinRead
)
...
func testServiceInstancesUpsert(t *testing.T) {
	...
	if len(mySQLServiceInstanceUniqueColumns) == 0 {
