...
var (
	stackDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Name`: `varchar`, `Description`: `varchar`}
	_            = bytes.MinRead
)
...
func testStacksUpsert(t *testing.T) {
	...
	if len(mySQLStackUniqueColumns) == 0 {
