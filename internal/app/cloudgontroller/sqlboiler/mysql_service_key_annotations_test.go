...
var (
	serviceKeyAnnotationDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `ResourceGUID`: `varchar`, `KeyPrefix`: `varchar`, `Key`: `varchar`, `Value`: `varchar`}
	_                           = bytes.MinRead
)
...
func testServiceKeyAnnotationsUpsert(t *testing.T) {
	...
	if len(mySQLServiceKeyAnnotationUniqueColumns) == 0 {
