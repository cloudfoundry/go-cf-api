// +build psql
//go:generate mockgen -source=$GOFILE -destination=mocks/kpack_lifecycle_data.go
...
func FindKpackLifecycleDatum(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*KpackLifecycleDatum, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"kpack_lifecycle_data\" where \"id\"=$1", sel,
	)
...
func (q kpackLifecycleDatumQuery) Insert(o *KpackLifecycleDatum, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"kpack_lifecycle_data\" %sDEFAULT VALUES%s"
		}
...
func (q kpackLifecycleDatumQuery) Update(o *KpackLifecycleDatum, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"kpack_lifecycle_data\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, kpackLifecycleDatumPrimaryKeyColumns),
		)
...
func (q kpackLifecycleDatumQuery) UpdateAllSlice(o KpackLifecycleDatumSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"kpack_lifecycle_data\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, kpackLifecycleDatumPrimaryKeyColumns, len(o)))
...
func (q kpackLifecycleDatumQuery) Delete(o *KpackLifecycleDatum, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"kpack_lifecycle_data\" WHERE \"id\"=$1"
...
func (q kpackLifecycleDatumQuery) DeleteAllSlice(o KpackLifecycleDatumSlice, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"kpack_lifecycle_data\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, kpackLifecycleDatumPrimaryKeyColumns, len(o))
...
func KpackLifecycleDatumExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"kpack_lifecycle_data\" where \"id\"=$1 limit 1)"
...
func (o *KpackLifecycleDatum) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(kpackLifecycleDatumPrimaryKeyColumns))
			copy(conflict, kpackLifecycleDatumPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"kpack_lifecycle_data\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert kpack_lifecycle_data")
	}
