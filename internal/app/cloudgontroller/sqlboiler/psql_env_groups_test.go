...
var (
	envGroupDBTypes = map[string]string{`ID`: `integer`, `GUID`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `Name`: `text`, `EnvironmentJSON`: `text`, `Salt`: `text`, `EncryptionKeyLabel`: `character varying`, `EncryptionIterations`: `integer`}
	_               = bytes.MinRead
)
