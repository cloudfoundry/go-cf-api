...
var (
	serviceInstanceOperationDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `ServiceInstanceID`: `int`, `Type`: `varchar`, `State`: `varchar`, `Description`: `text`, `ProposedChanges`: `varchar`, `BrokerProvidedOperation`: `text`}
	_                               = bytes.MinRead
)
...
func testServiceInstanceOperationsUpsert(t *testing.T) {
	...
	if len(mySQLServiceInstanceOperationUniqueColumns) == 0 {
