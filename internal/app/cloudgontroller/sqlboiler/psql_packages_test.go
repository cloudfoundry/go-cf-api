...
var (
	packageDBTypes = map[string]string{`ID`: `integer`, `GUID`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `Type`: `text`, `PackageHash`: `text`, `State`: `text`, `Error`: `text`, `AppGUID`: `text`, `DockerImage`: `text`, `Sha256Checksum`: `text`, `DockerUsername`: `text`, `DockerPasswordSalt`: `text`, `EncryptedDockerPassword`: `character varying`, `EncryptionKeyLabel`: `character varying`, `EncryptionIterations`: `integer`}
	_              = bytes.MinRead
)
