...
func (o *ProcessAnnotation) SetResource(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Process) error {
	...
	updateQuery := fmt.Sprintf(
		...
		strmangle.WhereClause("`", "`", 0, processAnnotationPrimaryKeyColumns),
	)
...
func FindProcessAnnotation(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ProcessAnnotation, error) {
	...
	query := fmt.Sprintf(
		"select %s from `process_annotations` where `id`=?", sel,
	)
...
func (o *ProcessAnnotation) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `process_annotations` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `process_annotations` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, processAnnotationPrimaryKeyColumns))
		}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == processAnnotationMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for process_annotations")
	}
...
func (o ProcessAnnotationSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `process_annotations` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, processAnnotationPrimaryKeyColumns, len(o)))
...
var mySQLProcessAnnotationUniqueColumns = []string{
	"id",
	"guid",
}
...
func (o *ProcessAnnotation) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(processAnnotationColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLProcessAnnotationUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`process_annotations`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `process_annotations` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for process_annotations")
	}
	...
	uniqueMap, err = queries.BindMapping(processAnnotationType, processAnnotationMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for process_annotations")
	}
...
func (o *ProcessAnnotation) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `process_annotations` WHERE `id`=?"
...
func ProcessAnnotationExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `process_annotations` where `id`=? limit 1)"
