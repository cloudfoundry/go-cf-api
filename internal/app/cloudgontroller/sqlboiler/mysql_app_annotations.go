...
func FindAppAnnotation(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*AppAnnotation, error) {
	...
	query := fmt.Sprintf(
		"select %s from `app_annotations` where `id`=?", sel,
	)
...
func (q appAnnotationQuery) Insert(o *AppAnnotation, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `app_annotations` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `app_annotations` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, appAnnotationPrimaryKeyColumns))
		}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == appAnnotationMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for app_annotations")
	}
...
func (q appAnnotationQuery) Upsert(o *AppAnnotation, ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(appAnnotationColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLAppAnnotationUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`app_annotations`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `app_annotations` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for app_annotations")
	}
	...
	uniqueMap, err = queries.BindMapping(appAnnotationType, appAnnotationMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for app_annotations")
	}
