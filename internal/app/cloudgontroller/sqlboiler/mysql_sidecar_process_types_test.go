...
var (
	sidecarProcessTypeDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `Type`: `varchar`, `SidecarGUID`: `varchar`, `AppGUID`: `varchar`}
	_                         = bytes.MinRead
)
...
func testSidecarProcessTypesUpsert(t *testing.T) {
	...
	if len(mySQLSidecarProcessTypeUniqueColumns) == 0 {
