...
func testAppToManyAddOpDroplets(t *testing.T) {
	...
	foreigners := []*Droplet{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, dropletDBTypes, false, strmangle.SetComplement(dropletPrimaryKeyColumns, dropletColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Droplet{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddDroplets(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Droplets[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Droplets[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Droplets().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testAppToManySetOpDroplets(t *testing.T) {
	...
	err = a.SetDroplets(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetDroplets(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
...
func testAppToManyRemoveOpDroplets(t *testing.T) {
	...
	err = a.AddDroplets(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveDroplets(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	appDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `SpaceGUID`: `varchar`, `Name`: `varchar`, `DropletGUID`: `varchar`, `DesiredState`: `varchar`, `EncryptedEnvironmentVariables`: `text`, `Salt`: `varchar`, `MaxTaskSequenceID`: `int`, `BuildpackCacheSha256Checksum`: `varchar`, `EnableSSH`: `tinyint`, `EncryptionKeyLabel`: `varchar`, `EncryptionIterations`: `int`, `RevisionsEnabled`: `tinyint`}
	_          = bytes.MinRead
)
...
func testAppsUpsert(t *testing.T) {
	...
	if len(mySQLAppUniqueColumns) == 0 {
