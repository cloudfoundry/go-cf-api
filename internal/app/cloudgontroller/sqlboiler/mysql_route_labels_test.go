...
var (
	routeLabelDBTypes = map[string]string{`ID`: `int`, `GUID`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`, `ResourceGUID`: `varchar`, `KeyPrefix`: `varchar`, `KeyName`: `varchar`, `Value`: `varchar`}
	_                 = bytes.MinRead
)
...
func testRouteLabelsUpsert(t *testing.T) {
	...
	if len(mySQLRouteLabelUniqueColumns) == 0 {
