// +build psql
//go:generate mockgen -source=$GOFILE -destination=mocks/env_groups.go
...
func FindEnvGroup(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*EnvGroup, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"env_groups\" where \"id\"=$1", sel,
	)
...
func (q EnvGroupQuery) Insert(o *EnvGroup, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"env_groups\" %sDEFAULT VALUES%s"
		}
...
func (q EnvGroupQuery) Update(o *EnvGroup, ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"env_groups\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, envGroupPrimaryKeyColumns),
		)
...
func (q EnvGroupQuery) UpdateAllSlice(o EnvGroupSlice, ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"env_groups\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, envGroupPrimaryKeyColumns, len(o)))
...
func (q EnvGroupQuery) Delete(o *EnvGroup, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"env_groups\" WHERE \"id\"=$1"
...
func (q EnvGroupQuery) DeleteAllSlice(o EnvGroupSlice, ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"env_groups\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, envGroupPrimaryKeyColumns, len(o))
...
func EnvGroupExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"env_groups\" where \"id\"=$1 limit 1)"
...
func (o *EnvGroup) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(envGroupPrimaryKeyColumns))
			copy(conflict, envGroupPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"env_groups\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert env_groups")
	}
